...
import { adminAiAssistantMock } from '@/lib/mock/adminAiAssistant.mock';
import {
...
// No lesson save feature for admin
import {
  buildAssistantReply,
  buildMetrics,
  buildQaFromTopic,
...
export function useAiAssistant(options?: {
  ...
  const seed = adminAiAssistantMock;
  ...
  const sendPrompt = () => {
    ...
    window.setTimeout(() => {
      ...
      setStatusMessage(
        cost > 0
          ? `Generated · ${cost} credits used · Save or Share below`
          : 'Generated · Save or Share below',
      );
    }, 650);
  };
  ...
  const runMessageAction = (messageId: string, action: AiFollowUpActionId) => {
    ...
    if (action === 'save') {
      setStatusMessage(`Saved “${topic}” to your drafts.`);
      return;
    }
